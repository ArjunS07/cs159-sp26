...
def create_snapshot(store, *, name: str, seed: int = 42, train_fraction: float = .8,
    ...
    # Validation here deliberately catches storage/schema failures before any training run.
    transitions = load_transitions(store, rows, gamma=gamma)
    actions = np.concatenate([item.action.reshape(-1, item.action.shape[-1]) for item in transitions], axis=0)
    mean, std = actions.mean(0), actions.std(0).clip(1e-6)
    ...
    return DatasetSnapshot(
        ...
        action_std=tuple(float(x) for x in std), provenance={**payload, "n_transitions": len(transitions)} )
