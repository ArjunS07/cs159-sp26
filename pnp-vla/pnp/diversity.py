...
SOURCE_ACTION_HORIZON_EXPERIMENT = "pi05-source-action-horizon-20-v1"
...
SOURCE_HORIZON_MULTI_QUERY_EXPERIMENT = "pi05-source-u-horizon-multi-query-v1"
...
def run_source_action_horizon_worker(
        *, arm: str = "refinement", n_action_steps: int = 20,
        shard_count: int = 2, shard_index: int = 0,
        episodes_per_task: int = 1, episode_limit: int | None = None,
        manifest_hash: str = "", source_model_revision: str = "",
        experiment: str = SOURCE_ACTION_HORIZON_EXPERIMENT):
    """Collect a matched source-policy execution-horizon pilot.

    The default arm uses always-on K=5, steps (3,4), refine-last while executing the first 20
    actions from each generated 50-action chunk. ``arm='baseline'`` keeps the same probe and
    execution horizon without feedback. The two arms have distinct method/config rollout IDs.
    """
    from . import models
    from .config import Method, RolloutConfig
    from .experiments import (PRO_EXPANDED_EXPERIMENT,
    ...
    if arm not in {"refinement", "baseline"}:
        raise ValueError("arm must be 'refinement' or 'baseline'")
    if (isinstance(n_action_steps, bool)
            or int(n_action_steps) != n_action_steps or n_action_steps < 1):
    ...
    historical_rows = historical_rows[
        ...
        & historical_rows.pnp_k.eq(5)
        & historical_rows.pnp_step_indices.apply(lambda value: tuple(value or []) == (3, 4))
        & historical_rows[DIVERSITY_PAIR_KEYS].apply(tuple, axis=1).isin(manifest_identities)
    ].copy()
    if len(historical_rows) != expected_identities:
        raise ValueError(
            f"expected {expected_identities} exact historical 10-action baseline identities, "
            f"found {len(historical_rows)}")
    historical_sr = historical_rows.groupby("suite").success.mean().astype(float).to_dict()

    refinement = arm == "refinement"
    method = Method.REFINEMENT if refinement else Method.UNCERTAINTY
    config = RolloutConfig(
        pnp_steps=(3, 4), pnp_k=5, refine=refinement, refine_average=False,
    ...
    print({"source": f"{source_repo}@{source_revision}", "arm": arm,
           "n_action_steps": n_action_steps, "generated_chunk_size": 50,
           "target_cohort_size": len(manifest), "episodes_in_shard": len(episodes),
           "historical_comparator": (
               f"exact matched {PRO_EXPANDED_EXPERIMENT}/{Method.UNCERTAINTY}, "
               "10 executed actions")})
    policy, preprocess, postprocess = models.load_pi05(
        repo_id=source_repo, revision=source_revision)
    if n_action_steps > int(policy.config.chunk_size):
        raise ValueError(
            f"n_action_steps={n_action_steps} exceeds policy chunk_size={policy.config.chunk_size}")
    _run_collection(
        store=store, policy=policy, preprocess=preprocess, postprocess=postprocess,
        device=models.default_device(), experiment=experiment, episodes=episodes,
        methods=methods, cohort=f"source_action_horizon_{arm}",
        shard_count=shard_count, shard_index=shard_index,
        benchmark="libero_pro", driver="pi05_source_action_horizon",
        run_metadata={
            "source_model_repo_id": source_repo, "source_model_revision": source_revision,
            "bootstrap_manifest_hash": manifest_hash, "pnp_k": 5,
            "pnp_steps": [3, 4], "refine_average": False,
            "n_action_steps": n_action_steps, "arm": arm,
            "episodes_per_task": episodes_per_task, "episode_limit": episode_limit,
...
def build_source_fractional_refinement_methods():
    """Four paired arms for the reduced-strength P&P pilot."""
    from .config import Method, RolloutConfig
    ...
    methods = [
        ...
        (Method.REFINEMENT, RolloutConfig(**common, refine=True)),
        (Method.FRACTIONAL_M2, RolloutConfig(
            **common, refine=True, refine_horizon_m=2)),
        (Method.FRACTIONAL_M4, RolloutConfig(
            **common, refine=True, refine_horizon_m=4)),
    ]
    if len({name for name, _ in methods}) != len(methods):
        raise AssertionError("fractional-refinement arms need unique progress/logging names")
    return methods


def run_source_fractional_refinement_worker(
        *, shard_count: int = 4, shard_index: int = 0,
        episode_indices=(10,), episode_limit: int | None = None,
        manifest_hash: str = "", source_model_revision: str = "",
        experiment: str = SOURCE_FRACTIONAL_REFINEMENT_EXPERIMENT):
    """Run baseline/full/m=2/m=4 P&P on a balanced unseen LIBERO-PRO cohort.

    The default cohort is initialization index 10 for every one of the 10 tasks in 11 suites:
    110 identities not used by the prior indices-0..9 action-horizon study.  The two with-milk
    suites are excluded because their official assets only contain indices 0..9.  Every shard
    runs all four arms back-to-back with paired policy and perturbation noise.
    """
    from . import models
    from .experiments import (_prepare_libero_pro_expanded_episodes, _run_collection,
    ...
    if len(pilot_suites) != 11:
        raise ValueError(
            f"expected 11 fractional-pilot suites after excluding short assets; "
            f"found {len(pilot_suites)}: {pilot_suites}")
    ...
    if len(manifest) != expected_identities:
        raise ValueError(
            ...
            f"10 tasks x "
            f"{len(episode_indices)} initialization indices, found {len(manifest)}")
    if {int(ep["ep_idx"]) for ep in manifest} != set(episode_indices):
        raise ValueError("fractional-refinement manifest contains unexpected episode indices")
    episodes = identity_shard(manifest, shard_count, shard_index)
    if episode_limit is not None:
    ...
    print({
        ...
        "cohort": "11 suites x 10 tasks x unseen init-state indices",
    ...
    print("Fractional update: x <- x + [m/(10-step)] * (x_full_P&P - x)")
    print(f"{'arm':<24}{'m':>6}{'alpha@3':>12}{'alpha@4':>12}")
    for method, config in methods:
        m = config.refine_horizon_m
        alpha3 = (m / 7) if m is not None else (1.0 if config.refine else 0.0)
        alpha4 = (m / 6) if m is not None else (1.0 if config.refine else 0.0)
        print(f"{method:<24}{str(m if m is not None else '-'):>6}"
              f"{alpha3:>12.3f}{alpha4:>12.3f}")

    policy, preprocess, postprocess = models.load_pi05(
        repo_id=source_repo, revision=source_revision)
    if 10 > int(policy.config.chunk_size):
        raise ValueError("10 executed actions exceeds the loaded policy chunk size")
    _run_collection(
        store=store, policy=policy, preprocess=preprocess, postprocess=postprocess,
        device=models.default_device(), experiment=experiment, episodes=episodes,
        methods=methods, cohort="source_fractional_refinement",
        shard_count=shard_count, shard_index=shard_index,
        benchmark="libero_pro", driver="pi05_source_fractional_refinement",
        run_metadata={
            "source_model_repo_id": source_repo, "source_model_revision": source_revision,
            "bootstrap_manifest_hash": manifest_hash,
            "suites": pilot_suites,
...
def build_source_prefix_strength_methods(inner_strength: float = 0.5):
    """Paired hard-prefix and reduced-inner-excursion P&P pilot arms."""
    from .config import Method, RolloutConfig
    ...
    common = dict(
        ...
        save_time_uncertainty=True, skip_unused_renders=True, render_lead=2)
    methods = [
        (Method.PREFIX_REFINEMENT, RolloutConfig(
            **common, refine_prefix_only=True)),
        (Method.REDUCED_STRENGTH_REFINEMENT, RolloutConfig(
            **common, refine_inner_strength=float(inner_strength))),
    ]
    if len({name for name, _ in methods}) != len(methods):
        raise AssertionError("prefix/strength pilot arms need unique progress names")
    return methods
...
def run_source_prefix_strength_worker(
    ...
    if not source_model_revision:
        raise ValueError(
            "source_model_revision is required; pass it from the shared v2 Drive manifest")
    if not math.isfinite(float(inner_strength)) or not 0 < float(inner_strength) <= 1:
        raise ValueError("inner_strength must lie in (0, 1]")
    ...
    if int(policy.config.chunk_size) != 50:
        raise ValueError(
            f"prefix/strength design requires a 50-action chunk, found "
            f"{policy.config.chunk_size}")
    _run_collection(
        store=store, policy=policy, preprocess=preprocess, postprocess=postprocess,
        device=models.default_device(), experiment=experiment, episodes=episodes,
        methods=methods, cohort="source_prefix_strength",
        shard_count=shard_count, shard_index=shard_index,
        benchmark="libero_pro", driver="pi05_source_prefix_strength",
        run_metadata={
            "source_model_repo_id": source_repo, "source_model_revision": source_revision,
            "bootstrap_manifest_hash": manifest_hash,
...
def run_source_horizon_multi_query_worker(
        *, shard_count: int = 4, shard_index: int = 0,
        episodes_per_task: int = 10, episode_limit: int | None = None,
    ...
    per-iteration contraction profiles. The second arm independently queries the same immutable
    source checkpoint and executes the candidate with the lowest leading-horizon uncertainty.
    Both arms execute exactly ten actions before replanning.
    """
    from . import models
    from .config import Method, RolloutConfig
    from .experiments import (_prepare_libero_pro_expanded_episodes, _run_collection,
    ...
    if (isinstance(num_queries, bool) or int(num_queries) != num_queries
            or num_queries < 2):
        raise ValueError("num_queries must be an integer of at least 2")
    if (isinstance(selection_horizon, bool)
            or int(selection_horizon) != selection_horizon or selection_horizon < 1):
        raise ValueError("selection_horizon must be a positive integer")
    num_queries, selection_horizon = int(num_queries), int(selection_horizon)
    ...
    # Historical display is the corrected 10-action baseline, selected by its exact behavior
    # hash because this experiment also contains obsolete 20-action rows.
    historical_config = RolloutConfig(
        pnp_steps=(3, 4), pnp_k=5, refine=False, refine_average=False,
        n_action_steps=10)
    historical_hash = store.config_hash(store._logical_key(
        Method.UNCERTAINTY, historical_config))
    historical_rows = pd.DataFrame(store.fetch_all(
        "rollouts", "suite,task_idx,episode_idx,init_state_hash,status,success,method,config_hash",
        configure=lambda query: query.eq(
            "experiment", SOURCE_ACTION_HORIZON_EXPERIMENT).eq(
            "method", Method.UNCERTAINTY).eq("config_hash", historical_hash),
        order_by=("rollout_id",)))
    if historical_rows.empty:
        raise ValueError(
            f"no corrected 10-action baseline rows found in "
            f"{SOURCE_ACTION_HORIZON_EXPERIMENT}")
    historical_rows = historical_rows[
        historical_rows.status.eq("completed")
        & historical_rows[DIVERSITY_PAIR_KEYS].apply(tuple, axis=1).isin(
            manifest_identities)].copy()
    if historical_rows.duplicated(DIVERSITY_PAIR_KEYS).any():
        raise ValueError("duplicate corrected historical 10-action baseline identities")
    if len(historical_rows) != expected_identities:
        raise ValueError(
            f"expected {expected_identities} exact corrected historical baseline rows in "
            f"{SOURCE_ACTION_HORIZON_EXPERIMENT}, found {len(historical_rows)}")
    historical_sr = historical_rows.groupby("suite").success.mean().astype(float).to_dict()

    source_id = f"{source_repo}@{source_revision}"
    diagnostic_config = RolloutConfig(
        pnp_steps=(3, 4), pnp_k=5, n_action_steps=10,
        save_time_uncertainty=True, save_trajectory=True,
        skip_unused_renders=True, render_lead=2)
    multi_query_config = RolloutConfig(
        num_samples=num_queries, pnp_k=5, ms_probe_steps=(3, 4),
        selection_uncertainty_horizon=selection_horizon,
        candidate_set_id="|".join([source_id] * num_queries),
        n_action_steps=10, save_trajectory=True, save_generated_chunks=True,
        skip_unused_renders=True, render_lead=2)
    methods = [
    ...
    if not existing_rows.empty:
        ...
        existing_rows = existing_rows[
            existing_rows.config_hash.eq(expected)
            & existing_rows.status.eq("completed")
            & existing_rows[DIVERSITY_PAIR_KEYS].apply(tuple, axis=1).isin(
                shard_identities)]
    initial_tally = {} if existing_rows.empty else {
        (suite, method): [len(group), int(group.success.astype(bool).sum())]
        for (suite, method), group in existing_rows.groupby(["suite", "method"], sort=True)}
    ...
    print({
        ...
        "cohort": "13 suites x 10 tasks x episode indices 0..9",
    ...
    print("Periodic output: current per-suite SR, corrected historical SR, and U/C means.")
